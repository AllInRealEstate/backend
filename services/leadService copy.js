...
class LeadService {
   ...
   */
  /* v1
  async getFilteredLeads(filters = {}, workerId = null) {
    ...
    // 3. Pagination (Using GenericQueryHelper)
    const { page, limit, skip } = GenericQueryHelper.paginate(filters.page, filters.limit);

    // 4. Sorting (Map frontend keywords to Mongoose sort objects)
    let sort = { submittedAt: -1 }; // Default: Newest
    if (filters.sort === 'oldest') {
      sort = { submittedAt: 1 };
    } else if (filters.sort === 'priority') {
      sort = { priority: 1, submittedAt: -1 }; // High priority first
    }
    ...
    const [leads, total] = await Promise.all([
      Lead.find(query)
        ...
        .skip(skip)
        .limit(limit)
        .select('fullName email phoneNumber status priority inquiryType source submittedAt assignedTo unreadCount')
        .lean(),
  ...
  async getLeadById(leadId) {
    const lead = await Lead.findById(leadId)
      .select('fullName email phoneNumber status priority inquiryType source submittedAt assignedTo message')
      .populate('assignedTo', 'translations.en.name translations.he.name')
      .lean();
  ...
  */
  ...
  /* v2
  async getLeadById(leadId) {
    // 1. Fetch & Reset Counter
    // We use findByIdAndUpdate to reset the counter atomically.
    const lead = await Lead.findByIdAndUpdate(
      leadId,
      { unreadCount: 0 }, //  Reset count to 0
      { new: true }
    )
      .select('fullName email phoneNumber status priority inquiryType source submittedAt assignedTo message unreadCount')
      .populate('assignedTo', 'translations.en.name translations.he.name')
      .lean();
    ...
    // 2. Broadcast Reset to Super Admins
  ...
  async updateLeadStatus(leadId, status, adminInfo) {
    ...
    //  Superadmin + lead-room event
    socketService.emitStatusChange(leadId, oldLead.status, status, adminInfo);
    ...
    // Keep raw socket for the Lead Room (details page timeline)
    socketService.emitActivityLog(leadId, activity);

    //  Emit realtime status update events (lead room + superadmin)

    // Better: fetch old status before update (shown below).
    // We'll handle correctly by fetching oldLead before update.


    // 2. Notify Assigned Agent (if they didn't do it)
    if (lead.assignedTo && lead.assignedTo.toString() !== adminInfo.id) {
      ...
      for (const adminId of targetAdminIds) {
        ...
        //  DASHBOARD realtime event for assigned admin
        socketService.broadcastToRoom(`admin_${adminId}`, 'lead_status_changed', {
          leadId: lead._id,
          oldStatus: oldLead.status,        // will be set properly in Update 2.1 below
          newStatus: status,
          changedBy: adminInfo
        });
      }
    ...
    //  DASHBOARD realtime event for superadmins already exists in socketService.emitStatusChange
    // We'll call it properly in Update 2.1 below.
  ...
  async updateLeadStatus(leadId, status, adminInfo) {
    ...
    // Log Activity using Service (Handles Increment + Sockets)
  ...
  */
  ...
  async updateLeadPriority(leadId, priority, adminInfo) {
    ...
    //  Superadmin + lead-room event
    socketService.emitPriorityChange(leadId, oldLead.priority, priority, adminInfo);
    ...
    // Keep raw socket for the "Lead Room" (Live Dashboard)
    socketService.emitActivityLog(leadId, activity);

    // 2. Notify Assigned Agent (Persistent)
    if (lead.assignedTo && lead.assignedTo.toString() !== adminInfo.id) {
      const targetAdminIds = await getAdminIdsForWorkerProfile(lead.assignedTo);
    ...
    //  3. Notify Superadmins (Persistent)
    await notificationService.notifySuperAdmins(
      'PRIORITY_CHANGE',
      TITLES.PRIORITY_CHANGE,
      BODIES.PRIORITY_CHANGED(adminInfo.name, lead.fullName, priority),
  ...
  */
  ...
  async updateLeadPriority(leadId, priority, adminInfo) {
    ...
    // ❌ REMOVED: socketService.emitActivityLog(leadId, activity); <--- THIS WAS THE ERROR
  ...
  async assignLead(leadId, teamMemberId, adminInfo) {
    ...
    //  Immediate safety throw to stop execution if the ID is wrong
    if (!lead) throw new AppError('Lead not found after update attempt', 404);

    let workerName = 'Unassigned';
    //  Optional chaining to handle the case where assignedTo might be null
    ...
    //  1. Handle OLD assignee (unassignment/reassignment)
    if (oldAssignee && oldAssignee.toString() !== assignedTo?.toString()) {
      ...
      for (const oldAdminId of oldAdminIds) {
        // Emit access revoked (kicks them out of lead details page)
        socketService.emitLeadAccessRevoked(leadId, oldAdminId, assignedTo);

        // Emit dashboard refresh (removes from their list)
        socketService.broadcastToRoom(`admin_${oldAdminId}`, 'lead_assigned', {
          leadId: lead._id,
          assignedTo: assignedTo,
          assignedBy: adminInfo,
          unassigned: true // ← Flag to indicate removal
        });
...
*/
  ...
  async assignLead(leadId, teamMemberId, adminInfo) {
    ...
    if (oldAssignee && oldAssignee.toString() !== assignedTo?.toString()) {
      ...
      // ✅ FIX: Send Kick-out Signal ONCE (Outside the loop)
  ...
  async deleteLead(leadId, userId) {
    const lead = await Lead.findById(leadId).select('_id fullName email assignedTo').lean(); // ← ADD assignedTo
    if (!lead) throw new AppError(ERROR.LEAD_NOT_FOUND, 404);

    // Notify assigned admin BEFORE deleting
    if (lead.assignedTo) {
      const assignedAdminIds = await getAdminIdsForWorkerProfile(lead.assignedTo);

      for (const adminId of assignedAdminIds) {
        // Emit to assigned admin's dashboard
    ...
    // Emit to superadmin + lead room
    socketService.emitLeadDeleted(leadId, userId);
  ...
  */
  ...
  /*
  //v1
  async bulkDeleteLeads({ leadIds, selectAll, filters, excludedIds, userId }) {
    let targetIds = [];
    ...
    // Fetch all leads with their assignments BEFORE deleting
    const leadsWithAssignments = await Lead.find({ _id: { $in: targetIds } })
      .select('_id assignedTo')
      .lean();

    // Collect unique assigned admin IDs
    ...
    // Notify all affected admins BEFORE deletion
    ...
    // Emit to superadmin
    socketService.emitBulkLeadsDeleted(targetIds, userId);

    // Persistent Notification for Super Admins
    await notificationService.notifySuperAdmins(
      'BULK_DELETE',
      TITLES.BULK_DELETE,
      BODIES.BULK_DELETE(targetIds.length),
      { count: targetIds.length, actorId: userId },
      userId
    )
  ...
  async bulkDeleteLeads({ leadIds, selectAll, filters, excludedIds, userId }) {
    ...
    // Notify all affected admins BEFORE deletion (UI Refresh)
    for (const workerId of affectedWorkerIds) {
      const adminIds = await getAdminIdsForWorkerProfile(workerId);
      ...
      for (const adminId of adminIds) {
        socketService.broadcastToRoom(`admin_${adminId}`, 'bulk_leads_deleted', {
          ...
          leadIds: targetIds,
          deletedBy: userId,
          silent: true // ✅ Updates their grid without an extra toast
        });
      }
    }
    ...
    // 🔴 CHANGE: Replace the generic emit with a SILENT broadcast for Super Admins
    // Old: socketService.emitBulkLeadsDeleted(targetIds, userId); 
    
    // New: Broadcast silent refresh to all Super Admins
    socketService.broadcastToRoom('superadmin', 'bulk_leads_deleted', {
      count: targetIds.length,
      leadIds: targetIds,
      deletedBy: userId,
      silent: true // 👈 This ensures the grid updates, but the toast is suppressed
    });

    // Persistent Notification for Super Admins (Shows the Blue Toast)
    await notificationService.notifySuperAdmins(
      'BULK_DELETE',
      TITLES.BULK_DELETE,
      BODIES.BULK_DELETE_LOG(targetIds.length),
      { count: targetIds.length, actorId: userId },
      userId // ✅ Excludes the actor (Ward) from the Blue Toast
    );
  ...
  async bulkDeleteLeads({ leadIds, selectAll, filters, excludedIds, userId }) {
      ...
      for (const adminId of adminIds) {
        ...
        if (adminId.toString() !== userId.toString()) {
          await notificationService.createNotification(
            ...
            { count: countForWorker, actorId: userId }
          );
        }
      }
    }
    ...
    // This emits to 'lead_{id}' room, forcing anyone on the details page to leave
    targetIds.forEach(leadId => {
      socketService.emitLeadDeleted(leadId, userId);
    });
...
*/
  ...
  /**
   * Create lead manually (admin/superadmin)
   */

  /*
  async createLeadManually(leadData, adminUser) {
    if (!leadData.fullName || !leadData.email || !leadData.phoneNumber || !leadData.inquiryType) {
    ...
    //  Scenario A: Unassigned - Notify Superadmins
    if (!lead.assignedTo) {
      await notificationService.notifySuperAdmins(
        'LEAD_CREATED',
        TITLES.LEAD_CREATED,
    ...
    //  Scenario B: Assigned - Notify Agent + Superadmins
    else {
      const targetAdminIds = await getAdminIdsForWorkerProfile(lead.assignedTo);
      ...
      for (const adminId of targetAdminIds) {
        await notificationService.createNotification(
          ...
          'LEAD_ASSIGNED',
          TITLES.LEAD_ASSIGNED,
          `${actorName} created and assigned a new lead to you: ${lead.fullName}`,
          { leadId: lead._id, actorId: adminUser?._id }
        );
      }
      ...
      await notificationService.notifySuperAdmins(
        'LEAD_CREATED',
        'New Assigned Lead',
        `${actorName} created lead ${lead.fullName} and assigned it.`,
        { leadId: lead._id, actorId: adminUser?._id }
      );
    }
  ...
  /**
   * Bulk Assign Leads (with realtime dashboard + lead-room activity refresh)
   */
  /*
  async bulkAssignLeads({ leadIds, selectAll, filters, excludedIds, assignedTo, adminInfo }) {
    ...
    // 2) Resolve Team Member Name (for Activity Log)
    let workerName = 'Unassigned';
    if (assignedTo) {
    ...
    // 4) Bulk Log Activity (insertMany)
    const now = new Date();
    ...
    //  Option 1: Lead-room activity refresh ping (lightweight)
    // Anyone currently inside lead_<id> room (lead details page) will refetch activities
    for (const leadId of targetIds) {
      socketService.broadcastToRoom(`lead_${leadId}`, 'lead_activity_refresh', { leadId });
    }

    // 5) Notify assigned agent(s) + realtime dashboard refresh
    if (assignedTo) {
      const targetAdminIds = await getAdminIdsForWorkerProfile(assignedTo);

      // Persistent notifications + realtime bell
      for (const adminId of targetAdminIds) {
        await notificationService.createNotification(
          adminId,
          'BULK_ASSIGNMENT',
      ...
      //  Realtime dashboard refresh for assigned admin(s)
      for (const adminId of targetAdminIds) {
        socketService.broadcastToRoom(`admin_${adminId}`, 'lead_assigned', {
          count: targetIds.length,
          assignedTo,
    ...
    //  Realtime dashboard refresh for superadmins
    ...
    // 6) Persistent superadmin notification (DB + realtime bell)
    await notificationService.notifySuperAdmins(
      'BULK_ASSIGNMENT',
      'Bulk Assignment Complete',
      `${adminInfo.name} bulk assigned ${targetIds.length} leads.`,
      { count: targetIds.length, actorId: adminInfo.id }
    );
